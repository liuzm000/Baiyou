...
public class HttpAsyncTaskUpload extends AsyncTask<String, Integer, String> {
	...
	private Map<String, String> files,header;
	...
	public HttpAsyncTaskUpload(Context context, CallBack callBack,Map<String, String> files) {
		this.context = context;
		this.callBack = callBack;
		this.files = files;
	}
	
	public HttpAsyncTaskUpload(Context context, CallBack callBack,Map<String, String> files,Map<String, String> header) {
		this.context = context;
		this.callBack = callBack;
		this.files = files;
		this.header = header;
	...
	protected String doInBackground2(String... params) {
        ...
        try {
            ...
            // 使用FileBody上传图片
            multipartContent.addPart(name, new FileBody(new File(filePath)));
//            multipartContent.addPart("userId", new StringBody("1"));
	...
	protected String doInBackground(String... params) {
		...
		try {
			...
			logInfo(tag, "getContentLength totalSize:" + totalSize);
	...
	private File handleCameraResult(String imageFilePath) {
		...
		try {
			...
			logInfo(tag, "handleCameraResult Exception" + e.toString());
		} finally {
			if (fOut != null) {
				try {
					bitmap.recycle();
